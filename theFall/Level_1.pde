...
class Level_1 {
  ...
  Level_1() {
    ...
    player_lvl1 = new Player(new PVector(385, Platform1.y - 50), 1);
  ...
  void play() { 
    if (startLevel) {
      ...
      // the player            
      player_lvl1.chekPlatform(L1_b1);
      player_lvl1.chekPlatform(L1_b2);
      player_lvl1.chekPlatform(L1_b3);
      player_lvl1.chekPlatform(L1_b4);
      player_lvl1.chekPlatform(L1_b5);
      player_lvl1.chekPlatform(L1_b6);
      player_lvl1.chekPlatform(L1_b7);
      player_lvl1.chekPlatform(L1_b8);
  ...
  void scrollPlatform() {
    ...
    if (player_lvl1.location.x >= width - valueX - 400) {
      // Rearrange valueX (-)
      valueX -= 7;
    }
    // If the player is on the left edge of the screen
    if (player_lvl1.location.x <= 0 - valueX + 400) {
      // Rearrange valueX (+)
      valueX += 7;
    }
  ...
  boolean isDead() {
    // If the player fall
    if (player_lvl1.location.y > height &&
      player_lvl1.location.x < Platform8.x + blockSize*4) {
      // Return true
      return true;
    } else {
      return false;
  ...
  boolean win() {
    // If the player fall
    if (player_lvl1.location.y > height &&
      player_lvl1.location.x > Platform8.x) {
      // Return true
      return true;
    } else {
      return false;
